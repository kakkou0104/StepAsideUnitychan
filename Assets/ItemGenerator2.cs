...
public class ItemGenerator2 : MonoBehaviour {
	...
	void Update () {
		...
		if (this.unitychan.transform.position.z >= startPos-distance) {
			int num = Random.Range(0,10);
			if (num <= 1) {
				//コーンをx軸方向に一直線に生成
				for (float j = -1; j <= 1; j += 0.4f) {
					GameObject cone = Instantiate (cornPrefab) as GameObject;
					cone.transform.position = new Vector3 (4 * j, cornPrefab.transform.position.y, startPos);
				}
			}else {
				//レーンごとにアイテムを生成
				for (int j = -1; j < 2; j++) {
